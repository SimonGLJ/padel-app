...
def generate_matches():
    ...
    for _ in range(1000):
        ...
        for c in range(len(players)//4):
            ...
            s += st.session_state.past_partnerships.get(tuple(sorted(h2)), 0) * 500
            for p1 in h1:
                for p2 in h2: s += st.session_state.past_opponents.get(tuple(sorted([p1, p2])), 0) * 10
...
if tid_raw and tid_raw != st.session_state.current_tid:
    ...
    if res.data:
        d = res.data[0]
        st.session_state.update({"current_tid": tid_raw, "players": d.get('players', []), "leaderboard": d.get('leaderboard', {}),
            "round_number": d.get('round_number', 1), "matches": d.get('matches', []), "history": d.get('history', []),
            "max_rounds": d.get('max_rounds', 7), "game_format": d.get('format', "Americano"), "partner_type": d.get('partner_type', "Skiftende makker"),
...
with st.sidebar:
    ...
    max_r = st.number_input("Max Runder", 1, 50, value=st.session_state.max_rounds)
...
with t2:
    if st.session_state.get("leaderboard"):
        df = pd.DataFrame.from_dict(st.session_state.leaderboard, orient='index')
        if not df.empty: st.dataframe(df[["KS", "V", "U", "T", "Point", "PF"]].sort_values(["Point", "V"], ascending=False), use_container_width=True)
