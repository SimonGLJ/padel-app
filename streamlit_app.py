...
if not st.session_state.current_tid:
    st.info("👋 Indtast et ID ovenfor for at starte.")
    st.stop()
...
with tab1:
    if not st.session_state.players:
        st.warning("⚠️ Tilføj spillere i menuen til venstre.")
    else:
        ...
        if is_over:
                ...
                if st.button("🎲 Generer Næste Kampe"):
                    ...
                    for i in range(len(st.session_state.players) // 4):
                        ...
                        if is_finale or g_format == "Mexicano":
                            ...
                            # Americano: Trækker 4 unikke spillere fra puljen
            ...
            for i, m in enumerate(st.session_state.matches):
                with st.container(border=True):
                    ...
                    s2 = c2.number_input(f"{' & '.join(m['H2'])}", 0, 32, value=int(m['S2']), key=f"s2_{i}")
                    if s1 + s2 != 32:
...
with tab3:
    ...
    if st.session_state.players:
        ...
        new_p = col1.number_input("Point", value=st.session_state.leaderboard[p_edit]["Point"])
        new_pf = col2.number_input("PF", value=st.session_state.leaderboard[p_edit]["PF"])
        new_v = col3.number_input("Vundne", value=st.session_state.leaderboard[p_edit]["V"])
