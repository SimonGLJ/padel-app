...
def save_to_supabase():
    # Vi samler alt i én funktion for at sikre sync
    payload = {
    ...
    try:
        ...
        st.toast(f"☁️ Gemt: {st.session_state.current_tid}")
...
if tid_input and tid_input != st.session_state.current_tid:
    res = conn.table("tournaments").select("*").eq("tournament_id", tid_input).execute()
    if res.data:
        d = res.data[0]
        st.session_state.update({
            "current_tid": tid_input, "players": d.get('players', []),
...
with st.sidebar:
    ...
    if g_format == "Mexicano" and st.session_state.players:
        ...
        for i in range(len(st.session_state.players) // 4):
            ...
            n = st.text_input(f"Bane {i+1}", value=default_n, key=f"c_in_{i}")
            c_names.append(n)
...
with tab1:
    if st.session_state.players:
        ...
        if is_over:
            ...
            df_f = pd.DataFrame.from_dict(st.session_state.leaderboard, orient='index').sort_values(["Point", "PF"], ascending=False)
            st.dataframe(df_f.reset_index().rename(columns={'index':'Spiller'}), use_container_width=True)
            st.download_button("📥 Eksporter", df_f.to_csv().encode('utf-8'), f"{tid_input}.csv")
        else:
